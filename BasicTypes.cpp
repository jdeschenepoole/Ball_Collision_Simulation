...
#include <iostream>		// for debug print out
...
CVector CVector::operator*(float scalar) const	// Times a scalar
{
	return CVector( this->x * scalar, this->y * scalar, this->z * scalar );
}
...
CVector CVector::operator/(float scalar) const	// Divided by a scalar
{
	return CVector( this->x / scalar, this->y / scalar, this->z / scalar );
}
	
CVector CVector::operator-() const	// Negative on all directions
{
...
{
	CVector vTemp;
	this->CrossProduct( *this, v, vTemp );
	return vTemp;
}

float CVector::Dot( const CVector &v )
{
	return CVector::DotProduct( *this, v );
}
...
{
	// Dot product is the squared length of the vector...
	//	is +ve when angle between vectors is acute ( <90 degrees )
	//	is -ve when angle between vectors is obtuse ( >90 degrees )
	//
