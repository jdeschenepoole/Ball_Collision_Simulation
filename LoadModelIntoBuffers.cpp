...
#include "CPlyFile4nt.h"

bool LoadModelIntoBuffer( std::wstring modelFilename, std::wstring &error )
{
	error = L"OK";
	HRESULT hr = S_OK;
	...
	for ( int index = 0; index != plyModel.GetNumberOfVerticies(); index++ )
	{
		...
		vertices[index].Color.x = getRand<float>( 0.0f, 1.0f );// 0.5f;		// "Red"
		vertices[index].Color.y = getRand<float>( 0.0f, 1.0f );//0.5f;		// "Green"
		vertices[index].Color.z = getRand<float>( 0.0f, 1.0f );//0.5f;		// "Blue"
		vertices[index].Color.w = 1.0f;		// "Alpha" or transparency	
	}
